...
export function AdminPanel({ onCreateTournament }: AdminPanelProps) {
  ...
  const initializeData = async () => {
    ...
    try {
      ...
      // Skip connectivity test for faster load
      console.log('[AdminPanel] Auth check successful, proceeding to fetch data')
  ...
  const fetchTournaments = async () => {
    try {
      ...
      // Skip connectivity test for faster loading
      console.log('Fetching full tournaments list...')
  ...
  const fetchStats = async () => {
    ...
    // Set default stats first to avoid waiting for data
    setStats({
      totalTournaments: 0,
      activeTournaments: 0,
      totalUsers: 0,
    ...
    try {
      // Get tournament stats (with timeout)
      const { data: tournamentData, error: tournamentError } = await Promise.race([
        supabase.from('tournaments').select('status, entry_fee, current_players'),
        new Promise<any>((_, reject) => setTimeout(() => reject(new Error('timeout')), 5000))
      ])
      
      console.log('Tournament stats result:', { tournamentData, tournamentError })
      ...
      // Get user count separately (with timeout)
      const { count, error: userError } = await Promise.race([
        supabase.from('users').select('id', { count: 'exact', head: true }),
        new Promise<any>((_, reject) => setTimeout(() => reject(new Error('timeout')), 5000))
      ])
      
      console.log('User count result:', { count, userError })
  ...
  if (error) {
    return (
      <div className="space-y-6">
        ...
        {/* Show diagnostic tools even with error */}
