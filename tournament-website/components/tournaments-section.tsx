...
const TournamentJoinTestDev =
  process.env.NODE_ENV !== "production"
    ? dynamic(() => import("./tournament-join-test").then(m => m.TournamentJoinTest), { ssr: false })
    : (() => null as any)
...
export function TournamentsSection({ onJoinTournament, onCreateTournament, isAdmin, tournaments: externalTournaments }: TournamentsSectionProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Debug component for troubleshooting (dev only, lazy) */}
      {process.env.NODE_ENV === 'development' && <TournamentJoinTestDev />}
    </div>
  )
}
